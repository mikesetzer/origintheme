	...
	class ThemeIsle_SDK_Feedback_Deactivate extends ThemeIsle_SDK_Feedback {
		...
		function get_html( $key ) {
			...
			foreach ( $options as $title => $attributes ) {
				$id   = $attributes['id'];
				$list .= '<li ti-option-id="' . $id . '"><input type="radio" name="ti-deactivate-option" id="' . $key . $id . '"><label for="' . $key . $id . '">' . __( $title, 'hestia' ) . '</label>';
				if ( array_key_exists( 'type', $attributes ) ) {
					$list        .= '<div>';
					$placeholder = array_key_exists( 'placeholder', $attributes ) ? __( $attributes['placeholder'], 'hestia' ) : '';
			...
			return '<div id="' . $this->product->get_key() . '">'
				   ...
				   . '<div class="actions">'
				   . get_submit_button(
					   __( $button_submit_before, 'hestia' ), 'secondary', $this->product->get_key() . 'ti-deactivate-yes', false, array(
						   'data-after-text' => $button_submit,
					   )
				   )
				   . get_submit_button( __( $button_cancel, 'hestia' ), 'primary', $this->product->get_key() . 'ti-deactivate-no', false )
				   . '</div></div>';
		}
